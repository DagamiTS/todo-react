...
function App() {
  ...
  useEffect(() => {
    axios.get('https://todo-react-api.herokuapp.com/api/lists?_expand=color&_embed=tasks')
      .then(({ data }) => setLists(data));
    axios.get('https://todo-react-api.herokuapp.com/api/colors')
  ...
  const onRemoveTask = (listId, taskId) => {
    if (window.confirm('Do you really want to delete this task?')) {
      ...
      setLists(newList);
      axios.delete(`https://todo-react-api.herokuapp.com/api/tasks/${taskId}`,)
  ...
  const onEditTask = (listId, taskObj) => {
    ...
    setLists(newList);
    axios.patch(`https://todo-react-api.herokuapp.com/api/tasks/${taskObj.id}`, { text: newTaskText })
      .catch(() => console.log('Error during updating task text!'));
  };
  ...
  const onCompleteTask = (listId, taskId, completed) => {
    ...
    setLists(newList);
    axios.patch(`https://todo-react-api.herokuapp.com/api/tasks/${taskId}`, { completed })
      .catch(() => console.log('Error during updating task status!'));
  };
    ...
    <div className="todo">
      <div className="todo__sidebar">
        ...
        <List items={lists || []} isRemovable onRemove={(id) => {
          ...
          setLists(newLists);
        }} onClickItem={list => history.push(`/lists/${list.id}`)} activeItem={activeItem} />
